			...
			<block v-for="(item, index) in favoriteList" :key="index">
				<view class="favorite-item" :class="{'b-b': index!==favoriteList.length-1}">
					<view class="image-wrapper" style="width: 180rpx;height: 180rpx;">
						<image :src="item.img " :class="[item.loaded]" mode="aspectFill" 
					...
					<view class="item-right">
						...
						<text class="price"><text v-if="item.originalPrice > (isVip?item.vipPrice:item.price)" style="text-decoration:line-through;color: #909399;margin-right: 20rpx;">¥{{item.originalPrice / 100.0}}</text>
