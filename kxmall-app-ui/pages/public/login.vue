	...
	<view >
		...
		<view class="flex align-center justify-center flex-direction" style="z-index: 1;">
			...
			<view v-if="mobileLogin" class="flex align-center justify-center flex-direction">
				...
				<view class="lem-text-df" style="margin-top: 30rpx;color: #bbbbbb;line-height: 30rpx;">
					登录即代表确认 <text style="color: #1F7AFF;">《注册协议》</text> <text style="color: #1F7AFF;">《隐私协议》</text>
				</view>
			...
			<view v-else class="flex align-center justify-center flex-direction">
				...
				<view class="lem-text-df" style="margin-top: 420rpx;color: #bbbbbb;line-height: 30rpx;">
					登录即代表确认 <text style="color: #1F7AFF;">《注册协议》</text> <text style="color: #1F7AFF;">《隐私协议》</text>
				</view>
			</view>
	...
	export default {
		...
		onLoad(options) {
			...
			uni.login({
				...
				success: (wxres => {
					this.logining = false
					this.$api.request('user', 'thirdPartLogin', {
						loginType: 1,
		...
		methods: {
			...
			loginRegis(){
				...
				that.$api.request('user', 'phoneLoginOrRegister', {
					phone: that.phone,
					verifyCode: that.verifyCode,
					...
					uni.hideLoading()
					that.logining = false
					// //新人登录，如果邀请码则跳到抢购券页面 oldman 为true 老用户
					if(that.$store.state.InvitationCode){
						//老用户 邀请码就提示
						if(res.data.oldman){
							uni.showModal({
						...
						that.$store.commit('setStorageCode','')
					}else{
						uni.navigateBack({
							delta:1
						})
			...
			miniWechatLogin(e) {
				...
				that.$api.request('user', 'authPhone', param, failres => {
