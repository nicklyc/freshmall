		...
		<view v-if="storage" class="bg-white">
			...
			:style="'top:'+topHeight+'px;height:'+scollHeight+'px;'">
				<view class="bg-white flex flex-wrap padding-bottom padding-top-sm solid-bottom padding-left-xs">
					...
					:class="{'line-green': item.id === currentIded}">{{item.title}}</view>
				</view>
			</view>
			<view :style="'top:'+topHeight+'px;'"
			 style="padding-left: 140rpx;padding-top: 90rpx;">
				...
				style="width: 558rpx;height: 180rpx;border-bottom: solid #E6E6E6 1rpx;">
					<image :src="item.img" mode="aspectFill" class="round"
	...
	export default {
		...
		onReachBottom() {
			...
			this.$api.request('goods', 'getGoodsPageByStorage',{
				...
				uni.hideLoading()
				this.tlist = this.tlist.concat(res.data.items)
		...
		methods: {
			loadData(){
				...
				this.$api.request('category', 'categoryList').then( res => {
					...
					that.slist = that.slist.concat(res.data[0].childrenList)
					that.sTitle = that.slist[1].title
			...
			tabtap(item){
				...
				this.sTitle = this.slist[1].title
			...
			loadCartData(){
				...
				that.$api.request('cart', 'getCartList',{
					...
					for (var i = 0; i < res.data.length; i++) {
						cartIds.push(res.data[i].skuId)
						for (var j = 0; j < that.tlist.length; j++) {
							if(that.tlist[j].skuId == res.data[i].skuId){
								that.tlist[j].num = res.data[i].num
					...
					for (var j = 0; j < that.tlist.length; j++) {
						if(!cartIds.includes(that.tlist[j].skuId)){
							that.tlist[j].num = 0
