<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.type" clearable filterable style="width: 200px" class="filter-item" placeholder="请选择优惠券类型" >
        <el-option v-for="(item,index) in couponTypeMap" :key="index" :label="item.name" :value="item.value" />
      </el-select>
      <el-select v-model="listQuery.status" clearable filterable style="width: 200px" class="filter-item" placeholder="请选择优惠券状态" >
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      >
        <el-form-item label="优惠券名称" prop="title">
          <el-input v-model="dataForm.title" :disabled="dialogStatus === 'update'" clearable/>
        </el-form-item>
        <el-form-item label="优惠卷类型" prop="type">
          <el-select v-model="dataForm.type" :disabled="dialogStatus === 'update'" clearable filterable>
            <el-option v-for="(item,index) in couponTypeOptions" :key="index" :label="item.name" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="介绍" prop="description" >
          <el-input v-model="dataForm.description" :disabled="dialogStatus === 'update'" clearable/>
        </el-form-item>
        <el-form-item label="优惠券数量" prop="total">
          <el-input v-model="dataForm.total" :disabled="dialogStatus === 'update'" clearable>
            <template slot="append">张</template>
          </el-input>
        </el-form-item>
        <el-form-item label="每人限领" prop="limit">
          <el-input v-model="dataForm.limit" :disabled="dialogStatus === 'update'" clearable>
            <template slot="append">张</template>
          </el-input>
        </el-form-item>
        <el-form-item label="满减金额" prop="discount">
          <el-input v-model="dataForm.discount" :disabled="dialogStatus === 'update'" clearable>
            <template slot="append">元</template>
          </el-input>
        </el-form-item>
        <el-form-item label="最低消费" prop="min">
          <el-input v-model="dataForm.min" :disabled="dialogStatus === 'update'" clearable>
            <template slot="append">元</template>
          </el-input>
        </el-form-item>
        <el-form-item label="优惠卷状态" prop="status">
          <el-select v-model="dataForm.status" :disabled="dialogStatus === 'update'" clearable filterable>
        ...
        <el-form-item v-show="dataForm.timeType === 0" prop="days">
          <el-input v-model="dataForm.days" :disabled="dialogStatus === 'update'" clearable>
        ...
        <el-form-item v-if="dialogStatus === 'update'" v-show="dataForm.goodsType === 1" label="优惠类目">
          <el-input v-model="dataForm.categoryTitle" :disabled="dialogStatus === 'update'" clearable />
        </el-form-item>
